...
# LV mass, aung et al, n=16,923
lvm <-as.data.frame(fread("http://ftp.ebi.ac.uk/pub/databases/gwas/summary_statistics/GCST009001-GCST010000/GCST009396/AungN_31554410_LVM.txt.gz"))
lvm_iv<-subset(lvm, P<5e-8)
setnames(lvm_iv, old = c("SNP",  "EA", "NEA", "EAF", "BETA", "SE", "P"), 
         new = c("SNP",  "lvm_ea", "lvm_nea", "lvm_eaf", "lvm_beta", "lvm_se", "lvm_p"))
lvm_iv<-lvm_iv[,c("SNP",  "lvm_ea", "lvm_nea", "lvm_eaf", "lvm_beta", "lvm_se", "lvm_p")]
lvm_iv$lvm_ss <- 16923
rownames(lvm_iv) <- c()
rm(lvm)
